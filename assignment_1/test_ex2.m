...
centroids = zeros(K,size(rgb_data,2));
% choose first point randomly
centroids(1,:) = rgb_data(round(rand * (size(rgb_data,1)-1)+1),:);
i=2;
while i <= K
    centroids(i,:) = rgb_data(round(rand * (size(rgb_data,1)-1)+1),:);
    if( max(sum(bsxfun(@eq,centroids(1:i-1,:),centroids(i,:)))) < size(rgb_data,2) )
        i = i + 1;
    end
end
